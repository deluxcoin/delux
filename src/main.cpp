...
#if defined(NDEBUG)
# error "Delux cannot be compiled without assertions."
#endif
...
static inline bool IsValidTxAmount(int64_t In, int64_t Out) { return (In > Out); }
...
const string strMessageMagic = "Delux Signed Message:\n";
...
{
    int64_t nSubsidy = Params().PoWReward(nHeight);
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, Bignum nCoinAge, int64_t nFees)
{
    Bignum nSubsidy;
    int64_t nHeight = pindexPrev ? pindexPrev->nHeight + 1 : 0;

    nSubsidy = nCoinAge * Bignum(Params().StakeCoinYearReward(nHeight) * 33 / (365 * 33 + 8));
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t nTargetSpacing = Params().TargetSpacing(pindexLast->nHeight);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        int64_t nReward = GetProofOfWorkReward(pindex->pprev ? pindex->pprev->nHeight + 1 : 0, nFees);
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(pindex->pprev, nCoinAge * COIN, nFees);
