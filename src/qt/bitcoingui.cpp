    ...
    nWeight(0)
{
    ...
    setWindowTitle(tr("Delux"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/delux"));
    setWindowIcon(QIcon(":icons/delux"));
#else
...
void BitcoinGUI::restoreWindowGeometry()
{
    ...
    QSize size = settings.value("nWindowSize", QSize(700, 500)).toSize();
    if (!pos.x() && !pos.y())
    {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Delux address"));
    ...
    aboutAction = new QAction(tr("&About Delux"), this);
    aboutAction->setToolTip(tr("Show information about Delux"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Delux"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/delux"), tr("&Show / Hide"), this);
...
void BitcoinGUI::createToolBars()
{
    ...
    QWidget* toolbar_logo = new QWidget();
    toolbar_logo->setFixedSize(144, 144);
    toolbar_logo->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    toolbar_logo->setStyleSheet("QWidget { background-color: rgb(59,72,90); background-repeat: no-repeat; border-image: url(:/images/toolbar_logo); background-position: bottom center; margin: 6px; }");
    ...
    foreach(QAction *action, toolbar->actions()) {
        width = std::max(width, toolbar->widgetForAction(action)->width());
    }

    foreach(QAction *action, toolbar->actions()) {
        toolbar->widgetForAction(action)->setFixedWidth(width);
    }
}
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/delux_testnet"));
            setWindowIcon(QIcon(":icons/delux_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/delux_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Delux client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Delux client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Delux network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Delux address or malformed URI parameters."));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if (nLastCoinStakeSearchInterval && nWeight)
    {
        ...
        unsigned nEstimateTime = Params().TargetSpacing(nBestHeight) * nNetworkWeight / nWeight;
