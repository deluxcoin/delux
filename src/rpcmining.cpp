...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(params[0].get_int(), (int64_t)0);
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest, Bignum(nCoinAge), 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",     (uint64_t)GetProofOfWorkReward((int)nBestHeight, (int64_t)0)));
    ...
    obj.push_back(Pair("stakeinterest",    (uint64_t)Params().StakeCoinYearReward(nBestHeight)));
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    uint64_t nExpectedTime = staking ? (Params().TargetSpacing(nBestHeight) * nNetworkWeight / nWeight) : 0;
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Delux is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Delux is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Delux is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Delux is downloading blocks...");
