...
public:
    ...
    int64_t PoWReward(int nHeight) const {
        if (nHeight == 1)
            return (200000 * COIN);

        return nBasicPoWReward;
    }
    ...
    inline int64_t TargetSpacing(int nHeight) const { return nTargetSpacing; }
    inline int64_t TargetTimespan() const { return nTargetTimespan; }
